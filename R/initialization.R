new_initialization_of_parameters <- function(Y, W, V){
  ...
  newV <- ifelse(V == 0, 0, log(b1*V + b2*V2 + b3*V3 + b4*V4))
  newY <- logY - newV
...
Mix_correct_individual_sample <- function(Y, gc, reads, psi = 10, gamma =  0.8, steps = 50, down = 0.1){
  ...
  b3 <- bs[3]
  b4 <- bs[4]
  res <- Mix_gradient_descent_for_individual_sample(Y, gc, reads, a0, a1, a2, a3, a4, b1, b2, b3, b4, psi, gamma, steps, down)
